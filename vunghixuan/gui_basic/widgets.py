from PySide6.QtWidgets import QWidget, QGridLayout, QVBoxLayout, QLabel, QLineEdit, QPushButton, QComboBox, QMenuBar, QMenu, QToolBar, QListWidget, QListWidgetItem, QTableWidget, QAbstractItemView, QTableWidgetItem, QTabWidget,  QWidgetAction, QSpacerItem, QSpacerItem, QSizePolicy
...
class MyQTableWidget(QTableWidget):
    def __init__(self, parent=None):
        ...
        self.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers) # khóa chỉnh sửa text 
    ...
    #         # Lấy thông tin tên cột
    #         col_name = self.horizontalHeaderItem(col).text()
    #         # Lấy giá trị Ô
    #         cell_value = self.item(row, col).text()
    #         # Thông tin trên 1 dòng
    #         self.dic_rowData[col_name] = cell_value
            
        # print(self.dic_rowData)
        
    def showContextMenu(self, pos):
        contextMenu = QMenu(self)
        deleteAction = QAction("Xóa dòng", self)
        insertAction = QAction("Thêm dòng", self)
    ...
    def setEditTable(self, locked = None):
        if locked:
            ...
            self.setEditTriggers(QAbstractItemView.EditTrigger.DoubleClicked | QAbstractItemView.EditTrigger.SelectedClicked) # mở chỉnh sửa text 
    
    def setEditColumnTable(self, id_column):
        # Chỉ cho phép chỉnh sửa cột thứ id_colum        
        # for column in range(self.columnCount()):
        #     if column == id_column:
        #         self.setEditTriggers(QAbstractItemView.EditTrigger.DoubleClicked | QAbstractItemView.EditTrigger.SelectedClicked)
        for column in range(self.columnCount()):
            if column == id_column:
                self.horizontalHeaderItem(column).setFlags(Qt.ItemIsEditable)
            else:
                self.horizontalHeaderItem(column).setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)

    def insertRow(self):
        selectedRow = self.currentRow()
        self.insertRow(selectedRow)
    
    def upload_fileCSV(self, fileCSV):
        contents = fileCSV.dataTable[1:]
        headings = fileCSV.dataTable[0]
        self.upload_contents(contents)
        self.upload_headings(headings)
        self.auto_resize_columns() #tự động điều chỉnh cột

    def upload_list2D(self, list2D):
        contents = list2D[1:]
        headings = list2D[0]
        self.upload_contents(contents)
        self.upload_headings(headings)
    ...
    def setHeaderFont(self, size):
        ...
        header.setStyleSheet("QHeaderView::section { background-color: darkgreen; color: white; border: 1px solid gray;}")#font-weight: bold;
        # Sử dụng border: 1px solid gray; để không bị đổi nền
    ...
    def setFixedHeight(self, new_height):
        self.setFixedHeight(new_height)

    def set_row_hidden(self, ir, row_hidden):
        self.setRowHidden(ir, row_hidden)
    ...
    def write_fileData_by_class_CSVData(self, file_csv, list2D):
        file_csv.update_data([file_csv.headings]+list2D)
        file_csv.write_file(file_csv.dataTable)
...
class MyQLineEditSearch(QLineEdit):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.default_configuration()
