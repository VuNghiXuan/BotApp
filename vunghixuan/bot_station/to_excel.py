...
class ExcelWithTOC:
    ...
    def _update_toc(self):
        ...
        self.toc_sheet.range('A1').value = 'Danh Mục Sheets'
        for i, sheet in enumerate(self.wb.sheets):
            if sheet.name != self.toc_sheet_name:
                ...
                cell = self.toc_sheet.range(f'A{row_index_in_toc + 1}') # Bắt đầu từ hàng 2

                # Thêm số thứ tự vào giá trị của cell
                cell.value = f'{row_index_in_toc}. {sheet.name}'
